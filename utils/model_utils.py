...
def ChamferDistanceL1(output, gt):
    cham_loss = dist_chamfer_3D.chamfer_3DDist()
    dist1, dist2, _, _ = cham_loss(gt, output)
    cd_t = (dist1.mean(1) + dist2.mean(1))
    return cd_t

def ChamferDistanceL1_PM(output, gt):
    cham_loss = dist_chamfer_3D.chamfer_3DDist()
    dist1, _, _, _ = cham_loss(gt, output)
    return dist1.mean(1)
...
def get_repulsion_loss(pred, nsample=20, radius=0.07):
    ...
    dist_square = torch.max(torch.FloatTensor([1e-12]).expand_as(dist_square).to(pred.device), dist_square) #cuda 
...
def three_nn_upsampling(target_points, source_points):
    ...
    dist = torch.max(dist, torch.ones(1).to(target_points.device) * 1e-10) #cuda
...
def fps(data, number):
    '''
        ...
        number int
    '''
    fps_idx = ppn2.furthest_point_sample(data, number) 
