...
#
# Note:
# - Replace float -> float, kFloat -> kfloat in cubic_feature_sampling.cu
...
from utils.cubic_feature_sampling import CubicFeatureSamplingFunction
...
class CubicFeatureSamplingTestCase(unittest.TestCase):
    def test_neighborhood_size_1(self):
        ...
        self.assertTrue(
            gradcheck(CubicFeatureSamplingFunction.apply,
                      [ptcloud.float().cuda(), cubic_features.float().cuda()]))
    ...
    def test_neighborhood_size_2(self):
        ...
        self.assertTrue(
            gradcheck(CubicFeatureSamplingFunction.apply,
                      [ptcloud.float().cuda(), cubic_features.float().cuda(), 2]))
    ...
    def test_neighborhood_size_3(self):
        ...
        self.assertTrue(
            gradcheck(CubicFeatureSamplingFunction.apply,
                      [ptcloud.float().cuda(), cubic_features.float().cuda(), 3]))
