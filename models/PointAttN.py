...
class PCT_encoder(nn.Module):
    ...
    def forward(self, points):
        ...
        # points = gather_operation(points, idx_2)
        ...
        x2_d = x2_d.reshape(batch_size,self.channel*2,N//4)
...
class Model(nn.Module):
    ...
    def forward(self, x, gt=None, is_training=True, alpha=None):
        ...
        if is_training:
            ...
            gt_fine1 = gather_operation(gt.transpose(1, 2).contiguous(), furthest_point_sample(gt, fine.shape[1])).transpose(1, 2).contiguous()

            loss2, _ = calc_cd(fine, gt_fine1)
            gt_coarse = gather_operation(gt_fine1.transpose(1, 2).contiguous(), furthest_point_sample(gt_fine1, coarse.shape[1])).transpose(1, 2).contiguous()

            loss1, _ = calc_cd(coarse, gt_coarse)

            total_train_loss = loss1.mean() + (loss2.mean() + loss3.mean())*alpha

            return fine, loss3, total_train_loss
        else:
            cd_p, cd_t, f1 = calc_cd(fine1, gt, calc_f1=True)
            cd_p_coarse, cd_t_coarse = calc_cd(coarse, gt)
            return {'out1': coarse, 'out2': fine1,'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1}
            # return {'out1': fine, 'out2': fine1, 'cd_t_coarse': cd_t_coarse, 'cd_p_coarse': cd_p_coarse, 'cd_p': cd_p, 'cd_t': cd_t}
