...
class ECG_decoder(nn.Module):
    def __init__(self, args, num_input):
        ...
        self.num_fine = args.num_dense_points

        self.scale = int(np.ceil(self.num_fine / (self.num_coarse + num_input)))
...
class Model(nn.Module):
    def __init__(self, args, num_input=2048):
        super(Model, self).__init__()
        self.num_coarse = args.num_coarse_points
        self.num_points = args.num_input_points
        self.train_loss = args.loss
        self.encoder = PCN_encoder()
        self.decoder = ECG_decoder(args, num_input)
