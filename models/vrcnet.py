...
class Folding(nn.Module):
    ...
    def forward(self, point_feat, global_feat):
        ...
        grid_feat = self.grid.unsqueeze(0).repeat(batch_size, num_points, 1).transpose(1, 2).contiguous().to(point_feat.device)
...
class MSAP_SKN_decoder(nn.Module):
    ...
    def forward(self, global_feat, point_input):
        ...
        if self.points_label:
            id0 = torch.zeros(coarse_raw.shape[0], 1, coarse_raw.shape[2]).to(global_feat.device).contiguous()
            coarse_input = torch.cat( (coarse_raw, id0), 1)
            id1 = torch.ones(org_points_input.shape[0], 1, org_points_input.shape[2]).to(global_feat.device).contiguous()
...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        self.decoder = MSAP_SKN_decoder(num_fps=args.num_fps, num_fine=args.num_input_points, num_coarse=args.num_dense_points,
                                        num_coarse_raw=args.num_coarse_points, layers=layers, knn_list=knn_list,
