...
class PCN_decoder(nn.Module):
    def __init__(self, latent_dim, num_coarse, num_fine, scale, cat_feature_num, device):
        ...
        self.scale = scale
        self.grid = gen_grid_up(2 ** (int(math.log2(scale))), 0.05).to(device).contiguous() 
    ...
    def forward(self, x):
        ...
        grid_feat = grid.unsqueeze(0).repeat(batch_size, 1, self.num_coarse).contiguous().to(self.device)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.num_coarse = args.num_coarse_points
        self.num_dense = args.num_dense_points
        self.num_input = args.num_input_points
        self.latent_dim = args.latent_dim
        self.train_loss = args.loss
        self.scale = self.num_dense // self.num_coarse
        self.cat_feature_num = 2 + 3 + self.latent_dim

        self.device = args.device

        self.encoder = PCN_encoder(self.latent_dim)
        self.decoder = PCN_decoder(self.latent_dim, self.num_coarse, self.num_dense, self.scale, self.cat_feature_num, self.device)
